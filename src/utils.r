...
bash_script = function(
    ...
    out_roadmap_25 = paste0(out,'/roadmap_dist_25status')
    ifelse(!dir.exists(out_roadmap_25), dir.create(out_roadmap_25),'')
    'Ready\n' %>% cat
    ...
    roadmap_f_name = c(roadmap_f1,'roadmap_enh_merge.bed')
    roadmap_f = list.files(paste0(ann_path,'/roadmap_enh'),full.names=T)
    roadmap_f_path = c(roadmap_f,paste0(ann_path,'/roadmap_enh_merge.bed'))

    roadmap_bed = list.files(paste0(ann_path,'/roadmap_bed'),full.names=F)
    roadmap_bed_eid = lapply(roadmap_bed, function(x) strsplit(x,'\\_')[[1]][1])
    roadmap_bed_name = paste0('roadmap_',roadmap_bed_eid,'_25status')
    roadmap_bed_path = list.files(paste0(ann_path,'/roadmap_bed'),full.names=T)
    ...
    # Generate roadmap_enh bash script
    out2 = paste0(out,'/roadmap_dist/',tools::file_path_sans_ext(roadmap_f_name),'.tsv')
    bash2 = paste0(
        bedtools_sort, roadmap_f_path, ' | ',
        bedtools_closest_a, b_path,
        bedtools_closest_b, 'stdin',
        bedtools_out, out2
    )

    # Generate roadmap_bed bash script
    out3 = paste0(out,'/roadmap_dist_25status/',roadmap_bed_name,'.tsv')
    bash3 = paste0(
        bedtools_sort, roadmap_bed_path, ' | ',
        bedtools_closest_a, b_path,
        bedtools_closest_b, 'stdin',
        bedtools_out, out3
    )
